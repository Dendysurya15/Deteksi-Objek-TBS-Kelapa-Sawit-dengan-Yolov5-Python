...
def detect(opt):
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if show_vid:
                ...
                fontScale = 3
                #cv2.putText(im0, "FPS: " + str(int(1.0 / (t5-t1))), (15, 30), font, 1, color, 2, cv2.LINE_AA)
                cv2.putText(im0, str(count), org, font, fontScale, color, thickness, cv2.LINE_AA)
                cv2.putText(im0, str(count_now), (150,300), font, 1, (255, 255, 255), thickness, cv2.LINE_AA)
    ...
    # Print results
    f = open(str(Path(save_dir)) + "/" + str(project) + ".TXT", "a")
    ...
    if str(source).find("mp4") != -1:
        f.write('\n1')
        source_list =  str(source).split('/')
        f.write('\n2')
        source_list_ext = str(source_list[-1]).split('.')
        f.write('\n3')
        count_real_list = str(source_list_ext[0]).split('_')
        f.write('\n4')
        count_real = count_real_list[1]
        f.write('\n5')
        f.write('\n' + str(count))
        f.write('\n' + str(count_real))
        if int(count)==int(count_real):
            f.write('\n8')
            acc = 1
        elif int(count)>int(count_real):
            f.write('\n6')
            acc = float(count_real)/float(count)
        else:
            f.write('\n7')
            acc = float(count)/float(count_real)
        # f.write('\n6')

    f.write('\n10')
    f.write('\n11')
    f.write('\n' + str(model_list[-2]) + "\t" + str(imgsz) + "\t" + str(confidence) + "\t" + str(roi) + "\t" + str(iou) + "\t" + str(count) + "\t" + str(count_real) 
    + "\t" +  str(acc) + "\t" + str(source_list_ext[0]) + "\t" + (f'%.1f\t%.1f\t%.1f\t%.1f' % t))
