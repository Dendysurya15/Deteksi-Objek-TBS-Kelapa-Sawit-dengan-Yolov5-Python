...
def generate_report(content, path):
    ...
    if int(Ripe) != 0 and int(Overripe) != 0 and int(Unripe) != 0 and int(EmptyBunch) != 0 and int(Abnormal) != 0 and int(Kastrasi) != 0 and int(LongStalk) != 0:
        TotalJjg = int(Ripe) + int(Overripe) + int(Unripe) + int(EmptyBunch) + int(Abnormal) + int(Kastrasi) + int(LongStalk)
    ...
    # LOGGER.info(str(Ripe) + '\n' )
    # LOGGER.info(str(Overripe) + '\n')
    # LOGGER.info(str(Unripe) + '\n')
    # LOGGER.info(str(EmptyBunch) + '\n')
    # LOGGER.info(str(Abnormal) + '\n')
    # LOGGER.info(TotalJjg)
    # LOGGER.info(Ripe)
    ...
    LOGGER.info(date[0])
    dateImage = date[0]
    checkImgBest = os.path.isfile(str(path) + '/' + str(dateImage)  + '_best_.JPG')
    if(checkImgBest):
        image = ImgRl("img_inference/" + str(dateImage)  + '_best_.JPG')
    else:
        image = ImgRl("img_inference/no_image.png")
        
    checkImgWorst = os.path.isfile(str(path) + '/' + str(dateImage)  + '_worst_.JPG')
    if(checkImgWorst):
        image2 = ImgRl("img_inference/" + str(dateImage)  + '_worst_.JPG')
    else:
    ...
    doc = SimpleDocTemplate("pdf/" + no_plat +'_' + divisi + '_' + str(TotalJjg) + '.pdf', pagesize=letter)
...
def detect(opt):
    ...
    global best_ffb, worst_ffb, bad_ffb, good_ffb, last_mins
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                # pass detections to outputdeepsort
                t4 = time_sync()
            ...
            if show_vid:
                ...
                global count, count_now, unripe, ripe, overripe, empty_bunch, abnormal, long_stalk, kastrasi, countUnripe, countRipe, countOverripe, countEmptybunch, countAbnormal, countLongStalk, countKastrasi, prctgUR, prctgRP, prctgOV, prctgEB, prctgAB, prctgKS, no_line_log, path_plat
                color= (0, 255, 0)
                ...
                # def export_report():
                    ...
                    file_path = str(log_inference) + '/log.TXT'
            ...
            LOGGER.info(date)
            if worstCount > 5 and not worst_ffb:
                save_img_inference_sampling(im0, str(Path(os.getcwd())), date + '_worst_')
                worst_ffb = True
            elif bestCount > 5 and not best_ffb:
                LOGGER.info('44')
...
def save_inference_data(header, path):
    ...
    #     f.close()
    # LOGGER.info('62')
    path_inference_truk = path +'/log.TXT'
    # LOGGER.info(path_inference_truk)
    with open(path_inference_truk, 'r') as x:
        ...
        for line in content:
            ...
            if 'running' in line:
                ...
                content[count] = targetLine.replace(";running","")
