...
def detect(opt):
    global bad_ffb, good_ffb

    hasil, out, source, yolo_model, deep_sort_model, show_vid, save_vid, save_txt, imgsz, evaluate, half, project, name, exist_ok= \
        opt.hasil, opt.output, opt.source, opt.yolo_model, opt.deep_sort_model, opt.show_vid, opt.save_vid, \
    ...
    if ".mp4" in source:
            # Regular expression pattern to match the required variables
            pattern = r"r(\d+)-u(\d+)-o(\d+)-e(\d+)-a(\d+)-k(\d+)-l(\d+)\.mp4"

            vidPath = os.path.basename(source)
            #print(source)
            #print(vidPath)
            # Use regular expression to extract the required variables
            result = re.search(pattern, vidPath)

            # Extract the required variables from the result object
            rr = int(result.group(1))
            ru = int(result.group(2))
            ro = int(result.group(3))
            rem = int(result.group(4))
            ra = int(result.group(5))
            rk = int(result.group(6))
            rl = int(result.group(7))

            rj = rr+ru+ro+rem+ra+rk
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if show_vid:
                global count, count_now, unripe, ripe, overripe, empty_bunch, abnormal, long_stalk, kastrasi, countUnripe, countRipe, countOverripe, countEmptybunch, countAbnormal, countLongStalk, countKastrasi, prctgUR, prctgRP, prctgOV, prctgEB, prctgAB, prctgKS, prctgLS, percD, percJ, percR, percU,percO,percE,percA,percL,percK                
                color= (0, 255, 0)
            ...
            if not test:
                if time.time() > last_mins:
                    print(".")
                    if badCount > 3 and not bad_ffb:
                        save_img(im0, str(Path(os.getcwd())) , "/bad")
                        bad_ffb = True
                    elif goodCount > 3 and not good_ffb:
                        save_img(im0, str(Path(os.getcwd())), "/good")
                        good_ffb = True
    ...
    # Print results
    if test: #ubah lokasi save dan variable yg disave
        log_path = str(Path(str(hasil[0])))
        ...
        #f.write('\n' + str(model_list[-2]))
        if ".mp4" in source:
            vidname = os.path.basename(source)
        
        percR = round(abs(ripe-rr)/rj,2)
        percU = round(abs(unripe-ru)/rj,2)
        percO = round(abs(overripe-ro)/rj,2)
        percE = round(abs(empty_bunch-rem)/rj,2)
        percA = round(abs(abnormal-ra)/rj,2)
        percK = round(abs(kastrasi-rk)/rj,2)
        percL = round(abs(long_stalk-rl)/rj,2)
        jjgAI = ripe+unripe+overripe+empty_bunch+abnormal+kastrasi
        percJ = round(abs(jjgAI-rj)/(jjgAI+rj))
        percD = percR + percU + percO + percE +percA +percK + percL

        payload = "\n" + str(datetime.now(tz=tzInfo).strftime("%Y-%m-%d %H:%M:%S")) + "," + str(yolo_model_str[0]) + "," + str(vidname) + "," + \
        str(percD) + "," + str(percJ) + "," + str(percR)+ ","  + str(percU) + "," + str(percO)+ "," + str(percE)+ ","  + str(percA)+ "," + str(percK)+ ","  + str(percL)+ "," + \
        str(count) + "," + str(ripe) + ","  + str(unripe) + "," + str(overripe)+ "," + str(empty_bunch)+ ","  + str(abnormal)+ "," + str(kastrasi)+ ","  + str(long_stalk)+ "," + \
        str(rj) + "," + str(rr)+ ","  + str(ru) + "," + str(ro)+ "," + str(rem)+ ","  + str(ra)+ "," + str(rk) + "," + str(rl)+ "," + \
        str(imgsz) + "," + str(conf)+ ","  + str(roi) + "," + str(iou)
...
def count_obj(box ,w , h, id, cls, folder_kastrasi = ''):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hasil', nargs='+', type=str, default='/home/grading/Model', help='save the result')
